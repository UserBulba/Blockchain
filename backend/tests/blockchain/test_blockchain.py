...
def test_blockchain():
    ...
    assert blockchain.chain[0].current_hash == GENESIS_DATA["current_hash"]
...
def test_is_valid_chain_bad_genesis(blockchain_three_blocks):
    """?"""
    blockchain_three_blocks.chain[0].current_hash = "evil_hash"
...
def test_replace_chain_bad_chain(blockchain_three_blocks):
    """?"""
    blockchain = Blockchain()
    blockchain_three_blocks.chain[1].current_hash = "evil_hash"
